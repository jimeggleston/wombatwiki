#!python
root = "C:/Users/Jim/Dropbox/websites/bottle"
...
@app.route('/sitemap', method='POST')
@view('wikisearch.mako',output_encoding='utf-8')
def sitemap(top_page=FrontPage):
    def mapchildren(page, indent):
        pagechildren = re.findall(r'\b(?:[A-Z][a-z]+){2,}\b', WikiPage(page).get_text())
    ...
    # backsearch
    p = re.compile(r'\b%s\b' % self.wikipage.page)
    ...
    unmappedpages.sort()
    self.autotext += '__Pages outside main tree__\n *' \
                        + ('\n *'.join(unmappedpages) or '[None]') + '\n'
    wantedpages.sort()
    wikitext += '__Wanted pages__\n *' \
                        + ('\n *'.join(wantedpages) or '[None]') + '\n'
    wikihtml = wikiparser("SiteMap",wikitext)
    dict(action='show', page='Site Map', title='Site Map', wikihtml=wikihtml, errmsg='', breadcrumbs='')
    

bottle.run(app=application, host='localhost', port=80, server="tornado")
